...
const worker = new Worker<{ foo: string }>({
  ...
  heartbeatMs: 10_000,
  maxAttempts: 5,
  // Receives (attempt, error)
  backoff: (attempt, err) => {
    if (err && err.name === 'RateLimitError') return 60_000;
    return Math.min(30_000, 2 ** (attempt - 1) * 500);
  },
  async handler(job) {
...
- `backoff(attempt, error)` - function returning ms to wait before retrying (receives the thrown error)
...
- Throw `new UnrecoverableError('message')` inside your handler to skip retries and dead-letter immediately.
...
- `stalled` - `(jobId, groupId)` - emitted when a stalled job is recovered or failed due to max stalled count
- When a stalled job exceeds `maxStalledCount`, the worker also emits `failed` (and `error`) with the job in final failed status — same contract as a normal final failure
